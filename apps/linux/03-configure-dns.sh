...
echo -n "Retrieving the external IP address from the [$ingressName] ingress..."
while [[ -z $publicIpAddress ]]; do
  publicIpAddress=$(kubectl get ingress $ingressName -n $demoNamespace -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
